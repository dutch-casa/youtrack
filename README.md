...
Keys: `j/k` move, `/` changes the YouTrack issue query, `tab` switches details/comments/links/activity/attachments, `pgup/pgdn` scrolls the selected pane, `:` applies a YouTrack command to the selected issue, `g/G` top/bottom, `r` refresh, `q` quit.
