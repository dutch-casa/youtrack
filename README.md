...
Keys: `1-6` switch Issues, Knowledge Base, Help Desk, Agile Boards, Projects, and Users. Mouse clicks select sections and rows. The wheel moves the list on the left and scrolls content on the right. `j/k` move, `/` live-filters the current non-issue section or edits the issue query, `enter` opens tickets for the selected Help Desk project or sprints for the selected Agile board, `P` opens a project selector for issue filtering or Knowledge Base project scoping, `i` jumps to a specific issue ID, `o` opens the selected item in your browser, `c` adds a quick comment on issues, `w` adds a quick work item such as `45m implementation`, `n/p` moves between result pages, `tab` switches issue details/comments/links/activity/work/attachments, `pgup/pgdn` scrolls the selected pane, `:` applies a YouTrack command to the selected issue, `g/G` top/bottom, `r` refresh, `q` quit.
