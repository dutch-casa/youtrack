...
Knowledge-base article detail also needs to preserve the article title as the first visible document signal. Article code, project, and author are useful metadata, but they should not visually displace the title.
...
Render knowledge-base article bodies title-first: the article summary is the Markdown heading, while article ID, project, and author are secondary metadata below it.
...
Knowledge-base articles keep their title visible in the scroller, while metadata remains available and can wrap or truncate without becoming the primary signal.
...
- Put article code/project/author before the title: useful metadata, but it made the scroller feel like a record dump instead of a document.
